...
static void xremote_signal_success_view_draw_callback(Canvas* canvas, void* context)
{
    ...
    if (app_ctx->app_settings->orientation == ViewOrientationHorizontal)
    {
        ...
        xremote_canvas_draw_button_wide(canvas, model->ok_pressed, 68, 25, "Send", XRemoteIconEnter);
        xremote_canvas_draw_button_wide(canvas, model->back_pressed, 68, 45, "Retry", XRemoteIconBack);
    }
    else
    {
